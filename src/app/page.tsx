      ...
      {allTasks.map((t) => (
        ...
        >
          ...
          <div className="flex gap-2 *:text-sm mt-2">
            <Link href={"/add-note/" + t.id} className="bg-emerald-600 p-1 rounded-sm hover:bg-emerald-500 duration-100">update</Link>
            <Link href={"/delete-note/" + t.id} className="bg-rose-600 p-1 rounded-sm hover:bg-rose-500 duration-100">delete</Link>
          </div>
        </div>
      ))}
    </div>
